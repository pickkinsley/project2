    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-200 via-pink-100 to-rose-50 px-4 py-12">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="text-center py-6">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mb-3">PackSmart</h1>
          <p className="text-xl font-medium text-rose-600">Smart Packing Lists for Every Trip</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {FEATURES.map((f) => (
              <div key={f.title} className="bg-pink-50 rounded-xl p-4">
          ...
          <div className="space-y-3">
            {STEPS.map((s) => (
              <div key={s.number} className="flex items-start gap-3">
                <span className="shrink-0 w-7 h-7 rounded-full bg-emerald-400 text-white text-xs font-bold flex items-center justify-center">
                  {s.number}
                </span>
        ...
        {/* Tech stack */}
        <div className="bg-white/60 rounded-2xl px-6 py-5 text-rose-800 text-xs text-center leading-relaxed">
          Built with <span className="font-semibold text-rose-900">Go + Gin</span> backend ·{' '}
          <span className="font-semibold text-rose-900">MySQL</span> database ·{' '}
          <span className="font-semibold text-rose-900">React + Vite</span> frontend ·{' '}
          <span className="font-semibold text-rose-900">TanStack Query</span> for server state ·{' '}
          Deployed on <span className="font-semibold text-rose-900">AWS Lightsail</span>
        </div>
